...
export default function Leaderboard() {
  ...
  async function fetchLeaderboard(showLoadingIndicator = true) {
    ...
    if (data) {
      ...
      const processed = data.map(p => {
        ...
        let awardColor = "text-slate-700 border-slate-300 bg-slate-100";
        ...
        if (avg >= 80) {
          ...
          award = "BRONZE";
          awardColor = "text-amber-800 border-amber-400 bg-amber-100/60";
        }
  ...
  if (authorized === false) {
    return (
      <div className="min-h-screen bg-white text-slate-900 flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-slate-50 border border-slate-200 p-8 rounded-3xl max-w-md w-full shadow-lg">
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 p-4 md:p-8 font-sans print:bg-white print:text-black">
      <div className="max-w-5xl mx-auto print:max-w-full">
        
        {/* Top Header Interface */}
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 border-b border-slate-200 pb-6 print:hidden gap-6">
          <div>
            <h1 className="text-4xl md:text-6xl font-black italic tracking-tighter text-blue-600">
              LIVE <span className="text-slate-900">LEADERBOARD</span>
            </h1>
          </div>

          {/* Controls Panel Layout Configuration */}
          <div className="flex flex-wrap items-center gap-3 md:gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-200">
            <div className="flex bg-slate-200/60 p-1 rounded-xl border border-slate-300/50 w-full sm:w-auto justify-center">
              <button 
                onClick={() => setPrintLayout('with-points')}
                className={`flex-1 sm:flex-none px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${printLayout === 'with-points' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
              >
                With Points
              </button>
              <button 
                onClick={() => setPrintLayout('without-points')}
                className={`flex-1 sm:flex-none px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${printLayout === 'without-points' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
              >
                No Points
              </button>
            </div>
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <button 
                onClick={handlePrint}
                className="flex-1 sm:flex-none text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white px-4 md:px-5 py-2 rounded-xl uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
              >
                🖨️ Print
              </button>
              ...
              <button 
                ...
                className="flex-1 sm:flex-none text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white px-4 md:px-5 py-2 rounded-xl uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
              >
                📊 Excel
              </button>
              ...
              <button 
                ...
                className="text-xs font-bold bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 px-3 md:px-4 py-2 rounded-xl transition-all cursor-pointer shadow-sm"
              >
                {loading ? "..." : "🔄"}
              </button>
            </div>
        ...
        <div className="mb-8 space-y-4 print:hidden">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-xs font-extrabold uppercase text-slate-400 mr-2 tracking-wider">Medals:</span>
          ...
          {/* SDG Filter Dropdown */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-200">
            ...
            <select
              ...
              className="bg-white text-slate-800 border border-slate-300 rounded-xl px-3 py-2 text-xs font-bold w-full focus:outline-none focus:border-blue-500 shadow-sm"
            >
        ...
        <div className="space-y-4">
          {loading && standings.length === 0 ? (
            ...
            filteredStandings.map((item, index) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className="flex flex-col md:flex-row md:items-center justify-between p-4 md:p-6 rounded-2xl bg-slate-50 border border-slate-200 hover:border-slate-300 hover:shadow-md transition-all gap-4 print:border-slate-200 print:bg-white print:text-black print:p-4"
                >
                  {/* Left Section: Rank + Title + Info */}
                  <div className="flex items-start md:items-center gap-3 md:gap-6">
                    <span className="text-2xl md:text-3xl font-black text-slate-400 min-w-[2rem] shrink-0 print:text-black">
                      {index + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <h2 className="text-base md:text-xl font-bold uppercase tracking-tight text-slate-900 break-words print:text-black">
                        {item.project_name || "No Project Title"}
                      </h2>
                      <p className="text-xs md:text-sm text-slate-600 font-medium break-words mt-0.5">
                        Leader: {item.name || item.participant_name || "N/A"} {item.team_name ? `• ${item.team_name}` : ''}
                      </p>
                      
                      {/* Supervisor Display */}
                      {supervisorName && (
                        <p className="text-xs font-extrabold text-blue-600 uppercase mt-0.5">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        {(item.category || item.theme) && (
                          <span className="text-[10px] md:text-xs text-slate-500 font-semibold uppercase">
                            {item.category || item.theme}
                          </span>
                        )}

                        {item.program && (
                          <span className="text-[10px] font-extrabold bg-blue-50 border border-blue-200 text-blue-700 px-2 py-0.5 rounded uppercase">
                            {item.program}
                          </span>
                        )}
                        ...
                        {/* SDG Badge */}
                        {sdgGoal && (
                          <span className="text-[10px] font-extrabold bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-0.5 rounded uppercase">
                            🌐 {sdgGoal}
                          </span>
                        )}
                      </div>
          ...
          {filteredStandings.length === 0 && !loading && (
            <div className="text-center py-20 text-slate-400 font-bold uppercase tracking-widest border border-dashed border-slate-200 rounded-2xl bg-slate-50">
